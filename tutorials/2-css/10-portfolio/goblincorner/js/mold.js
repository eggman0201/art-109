class Mold {
  ...
  update(){
    ...
    // this got morphed into the above statements
...
//     this.fSensorPos.y = this.y + this.sensorDist*sin(this.heading);
    ...
    let index, l, r, f;
    index=4 * (d * floor(this.rSensorPos.x)) * (d * width) + 4 * (d * floor(this.rSensorPos.y));
    r=pixels[index];
    
    index=4 * (d * floor(this.lSensorPos.x)) * (d * width) + 4 * (d * floor(this.lSensorPos.y));
    l=pixels[index];
    
    index=4 * (d * floor(this.fSensorPos.x)) * (d * width) + 4 * (d * floor(this.fSensorPos.y));
    f=pixels[index];
